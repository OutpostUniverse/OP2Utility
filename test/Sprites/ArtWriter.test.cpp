...
TEST(ArtWriter, Empty)
{
	...
	EXPECT_NO_THROW(ArtWriter::Write(testFilename, ArtFile()));
	...
	EXPECT_NO_THROW(ArtWriter::Write(memoryWriter, ArtFile()));
}

TEST(ArtWriter, BlankFilename)
{
	EXPECT_THROW(ArtWriter::Write("", ArtFile()), std::runtime_error);
}
