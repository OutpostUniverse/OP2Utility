...
CHuffLZ::CHuffLZ(CBitStream *bitStream)
{
	...
	m_HuffTree = new CAdaptHuffTree(314);	// Construct the Adaptive Huffman tree
	m_BuffWriteIndex = 0;
	m_BuffReadIndex = 0;
	m_EOS = false;
...
CHuffLZ::CHuffLZ(int bufferSize, char *buffer)
{
	// Construct the BitStream object
	m_BitStream = new CBitStream(bufferSize, buffer);
...
int CHuffLZ::GetData(int bufferSize, char *buffer)
{
	int numBytesCopied;
	int numBytesTotal = 0;
...
const char* CHuffLZ::GetInternalBuffer(int *sizeAvailableData)
{
...
void CHuffLZ::FillDecompressBuffer()
{
...
int CHuffLZ::CopyAvailableData(int size, char *buff)
{
	int numBytesToCopy;
	int numBytesTotal = 0;
...
void CHuffLZ::WriteCharToBuffer(char c)
{
