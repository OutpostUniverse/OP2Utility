...
const char* CClmFile::GetInternalFileName(int index)
{
...
int CClmFile::ExtractFile(int index, const char *fileName)
{
	#pragma pack(push, 1)
	struct RiffHeader
	{
...
// Returns nonzero if successful and zero otherwise
bool CClmFile::CreateVolume(const char *volumeFileName, int numFilesToPack,
...
bool CClmFile::OpenAllInputFiles(int numFilesToPack, const char **filesToPack, HANDLE *fileHandle)
{
	int i;
...
int CClmFile::FindChunk(int chunkTag, HANDLE file)
{
	#pragma pack(push, 1)
	struct ChunkHeader
	{
...
bool CClmFile::WriteVolume(HANDLE outFile, 
						   int numFilesToPack, 
						   HANDLE *fileHandle, 
						   IndexEntry *entry,
