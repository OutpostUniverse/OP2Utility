...
// **************************************************************************************
// **************************************************************************************
CArchiveUnpacker::CArchiveUnpacker(const char *fileName)
{
...
CArchiveUnpacker::~CArchiveUnpacker()
{
...
int CArchivePacker::OpenOutputFile(const char *fileName)
{
...
void CArchivePacker::CloseOutputFile()
{
	if (m_OutFileHandle) CloseHandle(m_OutFileHandle);
	m_OutFileHandle = NULL;
}
...
bool CArchivePacker::ReplaceFileWithFile(const char *fileToReplace, const char *newFile)
{
...
int CMemoryMappedFile::MemoryMapFile(const char *fileName)
{
	// Open the file
...
// Release the memory mapping (if it exists)
void CMemoryMappedFile::UnmapFile()
{
	if (m_BaseOfFile) UnmapViewOfFile(m_BaseOfFile);
	if (m_FileMappingHandle) CloseHandle(m_FileMappingHandle);
	if (m_FileHandle) CloseHandle(m_FileHandle);
