...
CVolFile::CVolFile(const char *filename) : CArchiveFile(filename)
{
...
const char* CVolFile::GetInternalFileName(int index)
{
...
int CVolFile::GetInternalFileSize(int index)
{
...
int CVolFile::GetInternalFileOffset(int index)
{
	return m_Index[index].dataBlockOffset + 8;
}

int CVolFile::GetInternalFileNameOffset(int index)
{
...
bool CVolFile::CreateVolume(const char *volumeName, int numFilesToPack, 
							const char **filesToPack, const char **internalNames)
{
	SCreateVolumeInfo volInfo;
...
void CVolFile::CleanUpVolumeCreate(SCreateVolumeInfo &volInfo)
{
	int i;
...
bool CVolFile::WriteFiles(SCreateVolumeInfo &volInfo)
{
...
bool CVolFile::WriteHeader(SCreateVolumeInfo &volInfo)
{
	unsigned long numBytesWritten;
	int i;
...
bool CVolFile::OpenAllInputFiles(SCreateVolumeInfo &volInfo)
{
	int i;

	// Allocate space to keep track of all input files
	volInfo.indexEntry = new SIndexEntry[volInfo.numFilesToPack];
...
bool CVolFile::PrepareHeader(SCreateVolumeInfo &volInfo)
{
	int i;
	SIndexEntry *tempPtr;
	...
	for (i = 0; i < volInfo.numFilesToPack; i++)
	{
		...
		volInfo.indexEntry[i].compressionTag = 0x0100;
	...
	volInfo.indexTableLength = volInfo.numFilesToPack * sizeof(SIndexEntry);
...
bool CVolFile::CopyFileIntoVolume(HANDLE inputFile, int size)
{
...
bool CVolFile::WriteTag(int length, const char *tagText)
{
	unsigned long numBytesWritten;
	int buffer[2];
...
int CVolFile::ReadTag(int offset, const char *tagText)
{
...
bool CVolFile::ReadVolHeader()
{
	...
	m_Index = (SIndexEntry*)((char*)m_BaseOfFile + temp + 8);
