...
namespace Archives
{
	...
	void ClmFile::ExtractFile(std::size_t index, const std::string& pathOut)
	{
		...
		try
		{
			Stream::FileStreamWriter waveFileWriter(pathOut);
	...
	std::unique_ptr<Stream::SeekableStreamReader> ClmFile::OpenStream(std::size_t index)
	{
		CheckIndexBounds(index);
	...
	void ClmFile::CreateArchive(const std::string& archiveFilename, std::vector<std::string> filesToPack)
	{
		...
		std::vector<std::unique_ptr<Stream::FileStreamReader>> filesToPackReaders;
		...
		for (const auto& filename : filesToPack) {
			filesToPackReaders.push_back(std::make_unique<Stream::FileStreamReader>(filename));
		}
	...
	void ClmFile::ReadAllWaveHeaders(std::vector<std::unique_ptr<Stream::FileStreamReader>>& filesToPackReaders, std::vector<WaveFormatEx>& waveFormats, std::vector<IndexEntry>& indexEntries)
	{
		RiffHeader header;
	...
	uint32_t ClmFile::FindChunk(std::array<char, 4> chunkTag, Stream::SeekableStreamReader& seekableStreamReader)
	{
	...
	void ClmFile::WriteArchive(const std::string& archiveFilename,
		const std::vector<std::unique_ptr<Stream::FileStreamReader>>& filesToPackReaders,
		...
		const WaveFormatEx& waveFormat)
	{
		...
		Stream::FileStreamWriter clmFileWriter(archiveFilename);
