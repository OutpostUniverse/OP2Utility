...
OP2SpriteLoader::OP2SpriteLoader(std::string bmpFilename, std::shared_ptr<ArtFile> artFile) :
	bmpReader(bmpFilename), artFile(artFile) { }

void OP2SpriteLoader::ExtractImage(std::size_t index, const std::string& filenameOut) 
{
...
std::vector<Color> OP2SpriteLoader::GetPalette(const ImageMeta& imageMeta)
{
...
std::unique_ptr<Stream::FileSliceReader> OP2SpriteLoader::GetPixels(std::size_t startingIndex, std::size_t length)
{
	return std::make_unique<Stream::FileSliceReader>(bmpReader.Slice(startingIndex, length));
}

std::size_t OP2SpriteLoader::FrameCount(std::size_t animationIndex) const {
	return artFile->animations[animationIndex].frames.size();
}

std::size_t OP2SpriteLoader::LayerCount(std::size_t animationIndex, std::size_t frameIndex) const {
